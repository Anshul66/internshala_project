...
$username_err = $password_err = $Blood_err = $confirm_password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT id FROM userinternshala_login WHERE username = ?";
    ...
    if(empty(trim($_POST["blood"]))){
        $Blood_err = "Please enter Blood Group.";
    }
    else{
        $param_blood = trim($_POST["blood"]);
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($Blood_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO userinternshala_login (username, password, blood) VALUES (?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sss", $param_username, $param_password, $param_blood);
...
<div class="col-md-4 mx-auto">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <label>Blood Group</label>
                <input type="text" name="blood" class="form-control <?php echo (!empty($Blood_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $blood; ?>">
                <span class="invalid-feedback"><?php echo $Blood_err; ?></span>
            </div> 
            <div class="form-group">
